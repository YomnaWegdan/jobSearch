...
const searchCompanyByName = catchError(async (req, res, next) => {
    const companyNameQuery = req.query.name;
    ...
    try {
        const companies = await CompanyModel.find({
            companyName: { $regex: companyNameQuery, $options: 'i' }
        });
        ...
        return next(new appError('An error occurred while searching for companies', 500));
    }
});
