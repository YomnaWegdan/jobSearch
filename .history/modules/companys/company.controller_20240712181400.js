...
const searchCompanyByName = catchError(async(req , res , next)=>{
    const company = await CompanyModel.find({companyName:{$regex:req.query.name}})
    if (!companies.length) return next(new appError('No companies found with the given name', 404));
