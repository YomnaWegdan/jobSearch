...
const deleteJob = catchError(async (req, res, next) => {
    ...
    if (!job) return next(new appError('job not found', 404));
    res.status(200).json({ message: 'delete success', job });
})
const getAllJobsCompany = catchError(async (req, res, next) => {
    const jobs = await JobModel.find({ company: req.params.id }); //company: req.company.id
...
export{ 
    addJob , updateJob , deleteJob , getAllJobsCompany , getJobsOfSpacificCompany
}