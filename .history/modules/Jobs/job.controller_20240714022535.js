...
const applyToJob = catchError(async(req , res , next)=>{
    ...
    if (!jobId || !userId || !userTechSkills || !userSoftSkills) {
        return next(new appError('jobId, userId, userTechSkills, and userSoftSkills are required', 400));
    }

//     req.body.userResume = req.file.path;
//     req.body.userTechSkills= JSON.parse(userTechSkills)  // Parsing because it might come as a JSON string
//     req.body.userSoftSkills= JSON.parse(userSoftSkills)
...
const application = await ApplicationModel.create({
    jobId,
    userId,
    userTechSkills: JSON.parse(userTechSkills), // Ensure JSON parsing if required
    userSoftSkills: JSON.parse(userSoftSkills), // Ensure JSON parsing if required
    userResume: req.file.path
});

res.status(201).json({ message: 'Application submitted successfully', application });
});
const uploadResume = upload.single('userResume');
